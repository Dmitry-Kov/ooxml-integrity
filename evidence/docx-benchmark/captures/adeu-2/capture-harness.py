...
def capture(adapter, destination):
    ...
    (destination / "capture-harness.py").write_bytes(Path(__file__).read_bytes())
...
def target_normalization(root, action):
    ...
    for node in list(paragraph):
        ...
        # New revision timestamps are allowed with the declared new marker
        # pair, including adeu's UTC extension. Old revisions are never stripped.
        date_utc = "{http://schemas.microsoft.com/office/word/2023/wordml/word16du}dateUtc"
        if node.get(date_utc) == node.get(W + "date"):
            allowed_attrs.add(date_utc)
...
def evaluate(directory):
    ...
    for key, actual in (("protocol_sha256", digest(PROTOCOL)),
                        ("harness_sha256", digest(directory / "capture-harness.py")),
                        ("checker_source_sha256", tree_hash(ROOT / "src/ooxml_integrity"))):
    ...
    return {"schema": 1, "capture_sha256": digest(directory / "capture.json"),
            "evaluation_harness_sha256": digest(__file__),
...
def main():
    ...
    for name in ("evaluate", "verify"):
        ...
        p.add_argument("--result-name", default="evaluation.json")
    ...
    if args.command == "capture":
        ...
        if Path(args.result_name).name != args.result_name:
            raise ValueError("Result name must be a filename in the capture directory")
        target = args.directory / args.result_name
