...
def evaluate(directory):
    ...
    if declaration["protocol_sha256"] != bench.digest(bench.PROTOCOL):
        raise ValueError("Protocol drift")
    if declaration["harness_sha256"] != bench.digest(directory / "capture-harness.py"):
        raise ValueError("Captured execution code drift")
    ...
    results = []
    declared_cases = {c["id"]: c for c in declaration["cases"]}
    for case in receipt["cases"]:
        artifacts = directory / case["id"]
        if any(case.get(k) != value for k, value in declared_cases[case["id"]].items()):
            raise ValueError("Attempt task drift")
        if bench.digest(ROOT / case["path"]) != case["sha256"]:
            raise ValueError("Source drift")
        if case != json.loads((artifacts / "attempt.json").read_text()):
            raise ValueError("Per-attempt receipt drift")
        if json.loads((artifacts / "request.json").read_text()) != declaration["requests"][f"{case['action']}-{case['seed']}"]:
    ...
    return {"pipeline": "local-agent-v1", "capture_sha256": bench.digest(directory / "capture.json"),
            "evaluation_harness_sha256": bench.digest(__file__),
