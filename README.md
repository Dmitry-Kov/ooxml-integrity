...
One dependency (`lxml`), Python 3.9+. No model calls, no rendering, no network.
...
```yaml
- uses: Dmitry-Kov/docx-integrity@v0.1.0
  with:
    files: "out/**/*.docx"
...
**And a limit that cannot be engineered away.** The font a deck renders with
depends on what is installed where it is opened. Measuring with a
metric-compatible clone (Carlito for Calibri, Liberation Sans for Arial) is as
good as the original; measuring with something merely similar is not, and the
checker says which it did:
...
- **The pptx calibration is against LibreOffice, not PowerPoint.** Agreement to
  ...
  agrees. Running the same corpus through PowerPoint is the missing half, and
  the same applies to the metric-compatibility pairings, which cannot be
  verified on a machine where the real Microsoft fonts are not installed.
