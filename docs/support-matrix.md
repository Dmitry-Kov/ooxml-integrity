...
| --- | --- | --- |
| DOCX corpus | One byte-reproducible feature package; six clean and two defective real agent outputs; plus a [versioned synthetic beta tranche](../evidence/docx-beta/README.md) with 40 sources, 170 exactly labelled pairs, six document classes, and ten source files each from `python-docx`, LibreOffice, Word for Mac and Word for Windows. Ten pairs retain actual before/after Windows Word saves with hashes and semantic XML audits. | Customer distributions, Word Online, other Windows Word builds, an independently supplied commercial/internal generator (where available), independent human review of Windows labels, and positive labels for the rules marked not measured in the [rule-level result](../evidence/docx-beta/RESULTS.md). Synthetic equivalents are accepted by P0.5; client documents and dual human review are optional confidence extensions. |
| DOCX observed rendering | The key detached-comment case was inspected in Word for Mac. Twenty mutation cycles were converted by LibreOffice without it reporting the structural losses. [Windows Word 16.0.14334.20848 x64](../evidence/docx-beta/WINDOWS.md) opened/saved ten synthetic sources with preserved semantic XML; visual appearance was not scored. | Windows visual fidelity, Word Online, Google Docs and systematic DOCX checks in ONLYOFFICE. |
