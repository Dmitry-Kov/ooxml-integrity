...
| --- | --- | --- |
| Existing DOCX revisions | Separate [30-pair tranche](../evidence/docx-revisions/README.md): five synthetic sources, ten adeu 3.0.4 saves/edits, two Word Online edits and three Word for Windows no-op saves preserving review content; nine seeded defects (published checker: 7 detected, 2 missed; unreleased [FID009 follow-up](../evidence/docx-revision-text/README.md): 8 detected, 1 missed), and six accept/reject characterizations excluded from preservation metrics. Payload/author/date/nesting and reference checks are independent of checker findings. Windows table-width changes are recorded, with a scoped content comparison. | Unchanged layout, broader Office review operations/builds, independent customer material/review. The recorded count-neutral main-document substitution is now detected by FID009. Broader revision-identity loss and stripping a footnote revision while retaining its text remain gaps; `FID001` does not distinguish allowed acceptance from unrelated loss. |
