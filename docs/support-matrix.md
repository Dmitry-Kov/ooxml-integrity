...
This page describes the [0.4.2 candidate](releases/0.4.2.md), including its
changes from published `0.4.1`. No `0.4.2` release is published yet.
...
| Main-document revision text (0.4.2 candidate FID009) | **Partial** | With equal same-kind wrapper counts, compares nonempty literal text in direct paragraph-level insertions/deletions. IDs, formatting, metadata and wrapper boundaries are not identities. Nested/empty/non-text revisions skip that kind; searches have a fixed work budget. Incidental wording elsewhere can mask loss. [Coverage and explicit limits](../evidence/docx-revision-text/README.md). |
| Footnote/endnote revision presence (0.4.2 candidate FID010) | **Partial** | In conventional note parts, compares how many notes retain insertion/deletion markup within nonempty, equally populated normalized text groups (`w:t` plus `w:delText`). IDs and within-note fragment counts may change. Empty/changed groups are skipped; partial removal within one note and reassignment across identical note text can be missed. Wrong note-part roots prevent comparison. [Scope and coverage](../evidence/docx-note-revisions/README.md). |
...
| Effective font size, family, bold and italic | **Partial** | Resolves these four properties independently through run and paragraph defaults, active-level list styles, placeholder styles, master text styles and presentation defaults. Explicit false overrides inherited bold/italic. [Candidate-chain scope and font-selection regressions](pptx-run-properties.md). Shape-style `a:fontRef` selection is not implemented. East Asian and complex-script theme aliases still select the corresponding Latin major/minor family; script-specific supplemental faces and shaping are not resolved. |
...
| --- | --- | --- |
| Existing DOCX revisions | Separate [30-pair tranche](../evidence/docx-revisions/README.md): five synthetic sources, ten adeu 3.0.4 saves/edits, two Word Online edits and three Word for Windows no-op saves preserving review content; nine seeded defects (published checker: 7 detected, 2 missed; 0.4.2 candidate [FID009/FID010 follow-up](../evidence/docx-note-revisions/README.md): 9 detected, 0 missed), and six accept/reject characterizations excluded from preservation metrics. Payload/author/date/nesting and reference checks are independent of checker findings. Windows table-width changes are recorded, with a scoped content comparison. | Unchanged layout, broader Office review operations/builds, independent customer material/review. The recorded main-document substitution and note-wrapper removal are detected by FID009/FID010. Partial note revision removal and broader revision-identity loss remain gaps; `FID001` does not distinguish allowed acceptance from unrelated loss. |
...
| Browser runtime | The [browser demo](../demo/README.md) pins a released package in Pyodide with bundled metric-compatible fonts. [Recorded checks](../demo/VALIDATION.md) include initial macOS Chrome/Safari/Firefox/in-app smoke tests and automated Chromium tests on macOS and Linux: examples, uploads, coverage/Doctor, blocked runtime/font startup with reload recovery, injected request latency, narrow layout, and real 60-second worker termination with UI/reload recovery. Candidate wheels use an isolated preview; local adapter tests also compare output with the CLI. | Physical mobile devices, bandwidth throttling of the entire startup, browser memory exhaustion and full offline startup. Injected delay and blocked individual resources cover specific network failures only. Browser execution does not add Office rendering coverage. |
