...
| --- | --- | --- |
| Existing DOCX revisions | Separate [27-pair tranche](../evidence/docx-revisions/README.md): five synthetic sources, ten clean adeu 3.0.4 saves/edits, two clean Word Online edits, nine seeded defects (7 detected, 2 missed), and six accept/reject characterizations excluded from preservation metrics. Payload/author/nesting and reference checks are independent of checker findings. | Word for Windows capture with existing revisions, broad Office review operations, independent customer material/review. Count-neutral revision payload loss and stripping a footnote revision while retaining its text remain known misses; `FID001` does not distinguish allowed acceptance from unrelated loss. |
