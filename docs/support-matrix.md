...
| --- | --- | --- |
| DOCX corpus | One byte-reproducible feature package; six clean and two defective real agent outputs; plus a [versioned synthetic beta tranche](../evidence/docx-beta/README.md) with 30 sources, 120 exactly labelled pairs, six document classes, and ten source files each from `python-docx`, LibreOffice and Word for Mac. | Customer distributions, Word for Windows, Word Online, an independently supplied commercial/internal generator, dual label review, and positive labels for the rules marked not measured in the [rule-level result](../evidence/docx-beta/RESULTS.md). |
