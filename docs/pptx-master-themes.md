...
Different slide masters can declare different heading and body fonts. The
checker now resolves Latin major/minor fonts through each slide's own layout
and master in Transitional PresentationML. This closes P1 item 4: text is
measured using its owning master's scheme, independently of other slide
masters, notes masters and theme order in the ZIP.

The dependency is `slide → slideLayout → slideMaster → theme`. The reader
follows typed internal relationships to identify these parts. Microsoft describes the
...
That can hide overflow when the selected font is narrower, or report overflow
when it is wider. The error comes from selecting the wrong declared family, so it
requires a change to theme resolution.
...
- The input opened without a repair prompt and was neither edited nor resaved.
  Export used **File → Export → PNG → Save Every Slide**, 960 × 540. All eight exported
  PNGs were inspected individually at full size. PowerPoint produced these
  images directly; the fixture-authoring tool and checker were not used to
  render them.
...
families. Neither master's relationship points to this part, despite its
conventional name.
...
`PPT003` ERROR at `slide3/FIT_B_wide`, giving three false negatives and one false
...
Ten W's are 144.02pt in Courier New, versus 226.52pt in Arial. These values
were calculated from font tables; widths were not measured from Office pixels.
...
- Existing font availability, substitution confidence and 5% overflow tolerance
  still apply. Overflow measured with fallback fonts remains WARN/Estimated.
  These three large, exact-or-metric-font overruns qualify for the existing
  `PPT003` ERROR rule.
  ...
  Finding codes, thresholds, the baseline format and the JSON schema remain
  unchanged.
...
mutations test the reader separately from the Office renderer observations above.

## Limits and remediation

The observation set covers one Mac Office build. These cases have not been
checked on Windows or the web, or independently reviewed. Placeholder
inheritance remains Partial. Shape-style
...
Latin major/minor family. Native EA/CS font resolution is not implemented.
